...
class NotionClient:
    ...
    def create_notion_page(self, properties):
        ...
        headers = {
            ...
            "Authorization": "".join(["Bearer", " ", self.token]) # 注意空格, 之前没注意空格, 一直报错
        }
    ...
    def notion_property(self, content, price, category, date, counterparty, remarks, 
                        transaction_number="",
        ...
        properties = {
            ...
            "From": {
                "select": {
                    "name": payment_platform_dict[self.payment_platform]
                }
            },
            "Counterparty": {
                "rich_text": [
    ...
    def process_row(self, row):
        if self.payment_platform == "alipay":
            properties = self.notion_property(row["商品说明"], row["金额"], row["交易分类"], row["交易时间"], row["交易对方"],
                                    row["备注"], row["交易订单号"], row["商家订单号"], row["收/付款方式"])
        elif self.payment_platform == "wechatpay":
            properties = self.notion_property(row["商品"], row["金额(元)"], row["交易类型"], row["交易时间"], row["交易对方"],
                                    row["备注"], row["交易单号"], row["商户单号"], row["支付方式"])
        else:
