...
class MailClient:
    ...
    def connect(self):
        ...
        if self.imap_url == "imap.163.com":
            ...
            self.mail._simple_command("ID",'("name" "test" "version" "0.0.1")')  # 163邮箱需要设置
    ...
    def walk_message(part, count=0):
        ...
        if part.get_content_type() == 'text/html':
            ...
            tree = html.fromstring(part.get_payload(decode=True), parser=html.HTMLParser(encoding='utf-8'))
            ...
            for link in link_elements:
                url = link.get('href')
                if url and re.match('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', url):
                    ...
                    if error_message in response.text:
                        raise Exception(f"Request failed: 不要慌, {error_message}, 并且要重新发送密码邮件")
                    ...
                    with open(os.path.join('attachment',re.sub(r'[\\/*?:"<>|]', "", filename)), 'wb') as f:
...
def main():
    ...
    client = MailClient(username, password, imap_url, payment_platform="wechatpay") # payment_platform="wechatpay"
